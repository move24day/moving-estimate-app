...
# --- 이사 비용 견적 틀 명확히 출력 ---
st.subheader("📌 이사 비용 견적 상세 내역")

# DataFrame을 활용하여 깔끔하게 정리
import pandas as pd

price_df = pd.DataFrame({
    '항목': ['기본 이사 비용', '사다리차 비용', '특정일 추가 비용', '총 비용'],
    '금액(만원)': [
        f"{price_info['기본 비용']}",
        f"{price_info['사다리차 비용']}",
        f"{price_info['특정일 비용']}",
        f"{price_info['총 비용']}"
    ]
})
...
# 사다리차 비용의 상세 내역 별도 표시
if price_info['사다리차 세부']:
    st.markdown("**🪜 사다리차 비용 상세:**")
...
# 특정일 추가비용 상세 내역 표시
if special_days:
    st.markdown("**📅 특정일 추가 비용 적용 항목:**")
    special_day_details = " / ".join(special_days)
    st.write(f"- {special_day_details}")
else:
    st.write("📅 특정일 추가 비용 적용 사항 없음.")
