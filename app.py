...
storage_daily_fee = 7000 # 보관이사 차량 톤당 1일 보관료 (톤수 곱하기 필요)
...
# <<< 추가됨: 차량 기반 박스/바구니 개수 저장을 위한 세션 상태 초기화 >>>
...
default_values = {
    ...
    "long_distance_selector": long_distance_options[0], # 오류 수정된 키 사용
...
with tab2:
                ...
                if col_index < len(cols):
                    with cols[col_index]:
                        if item in items:
                            ...
                            widget_key = f"qty_{st.session_state.base_move_type}_{section}_{item}"
                            st.number_input(
                                label=f"{item} ({unit})", min_value=0, step=1, key=widget_key,
                            )
                        else:
                             with cols[col_index]:
    ...
    # 선택된 품목 및 총량 계산 (자동 박스 계산 로직 제거됨)
    current_selection_display = {}
    total_volume = 0
    total_weight = 0
    # calculated_boxes = ... # <<< 삭제됨 >>>
    ...
    if current_selection_display:
        ...
        st.info(f"📊 총 부피: {total_volume:.2f} m³ | 총 무게: {total_weight:.2f} kg") # 자동 박스 부피 제외됨
...
with tab3:
    ...
    # --- 차량 선택 ---
    # (차량 총량 재계산 로직은 탭2와 동일하게 자동 박스 부피 제외됨)
    ...
    with col_v2:
        ...
        # <<< 추가 시작: 선택된 차량 톤수 기준 박스/바구니 수량 계산 및 세션 저장 >>>
        st.session_state.final_box_count = 0  # 계산 전 초기화
        st.session_state.final_basket_count = 0 # 계산 전 초기화
        if selected_vehicle and "초과" not in selected_vehicle:
            try:
                ...
                st.warning(f"차량({selected_vehicle}) 톤수 분석 중 오류 발생 (박스/바구니 계산): {e}")
        # <<< 추가 끝 >>>
    ...
    # (스카이, 추가인원, 폐기물, 날짜 선택 등 옵션 부분은 변경 없음)
    ...
    if uses_sky_from or uses_sky_final_to:
        ...
        if uses_sky_from:
            with col_sky1:
                st.number_input("출발지 스카이 시간", min_value=2, step=1, key="sky_hours_from")
        if uses_sky_final_to:
            to_label = "최종 도착지" if is_storage else "도착지"
            with col_sky2:
                st.number_input(f"{to_label} 스카이 시간", min_value=2, step=1, key="sky_hours_final")
    ...
    with col_add3:
       ...
       if base_women_count > 0:
          st.number_input("빼는 여성 인원 🧑‍🔧", min_value=0, step=1, max_value=base_women_count, key="remove_women")
       else:
          st.caption("기본 여성 인원 없음")
          if "remove_women" in st.session_state:
    ...
    # (비용 계산 로직 자체는 이전과 대부분 동일하나, 박스 관련 로직 없음 확인)
    total_cost = 0
    ...
    if selected_vehicle:
        ...
        if removed_women_cost_total > 0:
             if base_info.get('housewife', 0) >= remove_women_count:
                 total_cost -= removed_women_cost_total
    ...
    if st.button("PDF 견적서 생성", disabled=not can_generate_pdf, key="pdf_generate_button"):
        if not selected_vehicle: st.error("PDF 생성을 위해 차량을 선택해주세요.")
            ...
            basic_data.append(["장거리", st.session_state.get("long_distance_selector", "미입력")]) # 오류 수정된 키 사용
            ...
            work_data.append(["추가/변경 인원", f"추가 남 {pdf_add_men}명, 추가 여 {pdf_add_women}명, 빼는 여 {pdf_remove_women}명"])
            # <<< 추가 시작: PDF에 예상 박스/바구니 수량 표시 >>>
