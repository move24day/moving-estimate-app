...
# <변경됨> 비용 계산 시작 시 세션 상태에서 최종 확정된 차량 가져오기

selected_vehicle_for_costing = st.session_state.final_selected_vehicle



if selected_vehicle_for_costing:
...
# 1. 기본 비용 (selected_vehicle_for_costing 사용)

base_move_cost_type = vehicle_prices.get(current_move_type, {})

base_info_cost = base_move_cost_type.get(selected_vehicle_for_costing, {"price": 0, "men": 0, "housewife": 0})
...
calculated_cost_items.append(["기본 이사 비용 (보관x2)", f"{base_cost_calculated:,}원", f"{selected_vehicle_for_costing} ({base_personnel_str})"])
...
calculated_cost_items.append(["기본 이사 비용", f"{base_cost_calculated:,}원", f"{selected_vehicle_for_costing} ({base_personnel_str})"])
...
# <변경됨> 사다리 크기 결정 시 selected_vehicle_for_costing 사용

ladder_size = get_ladder_vehicle_size(selected_vehicle_for_costing)
...
# <변경됨> 도착지(일반) 또는 최종 도착지(보관) 작업 비용 계산

# 보관이사일 경우: 최종 도착지 기준 ('final_to_method', 'final_to_floor')

# 일반이사일 경우: 도착지 기준 ('to_method', 'to_floor')

if is_storage:

to_method_key = 'final_to_method'

to_floor_key = 'final_to_floor'
...
to_method_key = 'to_method'

to_floor_key = 'to_floor'

to_hours_key = 'sky_hours_final' # 일반 이사도 도착지 스카이는 이 키 사용
...
# 4. 보관료
        ...
        if is_storage:
            vehicle_ton_for_storage = get_vehicle_tonnage(selected_vehicle_for_costing)
            if vehicle_ton_for_storage > 0:
                ...
                calculated_cost_items.append(
                    ["보관료", f"{storage_fee:,}원", f"{storage_days}일 ({selected_vehicle_for_costing})"]
                )
            else:
                calculated_cost_items.append(["보관료", "계산 오류", "차량 톤수 확인 필요"])

        # --- 추가 인원 비용 ---
        additional_men_cost = add_men * ADDITIONAL_PERSON_COST
        if additional_men_cost > 0:
            total_cost += additional_men_cost
            calculated_cost_items.append(["추가 남성 인원", f"{additional_men_cost:,}원", f"{add_men}명"])

        additional_women_cost = add_women * ADDITIONAL_PERSON_COST
        if additional_women_cost > 0:
            total_cost += additional_women_cost
            calculated_cost_items.append(["추가 여성 인원", f"{additional_women_cost:,}원", f"{add_women}명"])

        # --- 기본 여성 인원 제외 할인 적용 ---
        if remove_base_housewife and base_women_cost > 0:
            discount = ADDITIONAL_PERSON_COST
            total_cost -= discount
            calculated_cost_items.append(["기본 여성 인원 제외 할인", f"(-){discount:,}원", "적용됨"])
        ...
        # --- 날짜 할증 (이사 집중일 운영비) ---
        special_day_cost = sum(special_day_prices.get(date, 0) for date in selected_dates)
        if special_day_cost > 0:
            total_cost += special_day_cost
            calculated_cost_items.append(["날짜 할증", f"{special_day_cost:,}원", ", ".join(selected_dates)])

        # --- 비용 내역 표시 ---
        st.subheader("📊 비용 상세 내역")
        if calculated_cost_items:
            cost_df = pd.DataFrame(calculated_cost_items, columns=["항목", "금액", "비고"])
            st.dataframe(cost_df.style.format({"금액": "{}"}).set_properties(**{'text-align': 'right'}, subset=['금액']),
    ...
    can_generate_pdf = selected_vehicle_for_costing and (st.session_state.customer_name or st.session_state.customer_phone)

    if st.button("PDF 견적서 생성", disabled=not can_generate_pdf, key="pdf_generate_button"):
        if not selected_vehicle_for_costing:
            ...
            pdf_data = generate_pdf(st.session_state.to_dict(), calculated_cost_items, total_cost)
            if pdf_data:
